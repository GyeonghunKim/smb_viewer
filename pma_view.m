...
% Last Modified by GUIDE v2.5 07-Oct-2018 02:52:42
...
if cameraType == 'emCCD'
    ...
    xsize = 512;
    ysize = 512;
    film_length=(file_info.bytes)/xsize/ysize/2;
    disp(film_length);
end
...
if cameraType == "emCCD"
    for i=start_frame:film_length
        ...
        colormap(hot);
        drawnow
        set(handles.edit3, 'String', sprintf("%d / %d",i, film_length))
        if isStop
            isStop = false;
            start_frame = i+1;
            break;
    ...
    for i=start_frame:film_length
        one_frame = fread(fid_pma,[ysize,xsize], 'int16');
        imagesc(one_frame')
        colormap(hot);
        drawnow
        set(handles.edit3, 'String', sprintf("%d / %d",i, film_length))
...
global dirr
cd(dirr)
tif_name = strcat(file_name(1:end-4), sprintf('_%05d.tif', start_frame-1));
imwrite(one_frame, hot, tif_name);
...
cd(dirr);
fid_pma2 = fopen(file_name,'r');
mkdir(foldername)
cd(foldername)

if cameraType == "emCCD"
    for i=1:film_length
        one_frame = uint16(fread(fid_pma2,[ysize,xsize], 'uint8'));
    ...
    for i=1:film_length
        one_frame = uint16(fread(fid_pma2,[ysize,xsize], 'int16'));
        name_i = sprintf('%05d',i);
        tif_name_i = strcat(file_name(1:end-4),'_',name_i,'.tif');
        imwrite(one_frame', tif_name_i);
    end 
